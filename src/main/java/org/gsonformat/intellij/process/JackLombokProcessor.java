 ...
 */
public class JackLombokProcessor extends Processor {
    ...
    private void injectAnnotation(PsiElementFactory factory, PsiClass generateClass) {
        ...
        if (modifierList != null) {
            ...
            Pattern pattern2 = Pattern.compile("@.*?JsonIgnoreProperties");
    ...
    protected void generateField(PsiElementFactory factory, FieldEntity fieldEntity, PsiClass cls, ClassEntity classEntity) {
        if (fieldEntity.isGenerate()) {
            Try.run(new Try.TryListener() {
                @Override
                public void run() {
                    cls.add(factory.createFieldFromText(generateJackLombokFieldText(classEntity, fieldEntity, null), cls));
                }
    ...
    private String generateJackLombokFieldText(ClassEntity classEntity, FieldEntity fieldEntity, String fixme) {
